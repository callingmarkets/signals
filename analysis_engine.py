...
<div style="border:1px solid #e5e7eb;border-radius:8px;padding:20px;margin-bottom:16px;">
  ...
  <div style="background:#fef2f2;border-radius:8px;padding:16px;text-align:center;">
    ...
    article_html = f"""<p style="font-size:13px;color:#6b7280;margin-bottom:24px;">Week of {week_of} &nbsp;·&nbsp; CallingMarkets Signal Intelligence</p>

{stats_bar}

<h2>State of the Market</h2>
{chr(10).join(f'<p>{p.strip()}</p>' for p in market_state.split('\n\n') if p.strip())}

<h2>Signal Flips Worth Watching</h2>
{chr(10).join(f'<p>{p.strip()}</p>' for p in flips_section.split('\n\n') if p.strip())}

<h2>Setups to Watch</h2>
{chr(10).join(f'<p>{p.strip()}</p>' for p in setups_section.split('\n\n') if p.strip())}
...
def run():
    ...
    for sector, tickers in sorted(sectors.items()):
        ...
        synopsis_prompt = f"""CallingMarkets weekly sector synopsis. Date: {today}
...
News:
{chr(10).join(headlines) if headlines else 'No headlines.'}
